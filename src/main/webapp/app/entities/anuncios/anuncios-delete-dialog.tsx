...
export interface IAnunciosDeleteDialogProps extends StateProps, DispatchProps, RouteComponentProps<{ id: string }> {}

export const AnunciosDeleteDialog = (props: IAnunciosDeleteDialogProps) => {
  useEffect(() => {
  ...
  const confirmDelete = () => {
    props.deleteEntity(props.anunciosEntity.id);
  };
...
const mapStateToProps = ({ anuncios }: IRootState) => ({
  ...
  updateSuccess: anuncios.updateSuccess,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(AnunciosDeleteDialog);
