...
export const Ciudad = (props: ICiudadProps) => {
  ...
  const { ciudadList, match, loading, totalItems } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        {ciudadList && ciudadList.length > 0 ? (
              ...
              {ciudadList.map((ciudad, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {ciudad.photo ? (
                      <div>
                        {ciudad.photoContentType ? (
                          <a onClick={openFile(ciudad.photoContentType, ciudad.photo)}>
                            <img src={`data:${ciudad.photoContentType};base64,${ciudad.photo}`} style={{ maxHeight: '30px' }} />
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {ciudad.photoContentType}, {byteSize(ciudad.photo)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${ciudad.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${ciudad.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Ciudads found</div>
        )}
      </div>
      {props.totalItems ? (
